...
class User:
    ...
    def __init__(self, locale: str = "en",
        ...
        self._locale = locale
        self._seed = seed
        self._finance = Finance(seed=self._seed)
        self._generic = Generic(self._locale, self._seed)

        self._user_id_start = user_id_start
        self._user_id_end = user_id_end
        self._max_friends = max_friends
        self._friends = Friends(user_id_start, user_id_end, max_friends)
    ...
    def make_one_user(self, user_id: int = 0):
        ...
        user["company"] = self._finance.company()
        ...
        user["friends"] = self._friends.make_friends()
        sample_size = random.randint(0, 5)
        user["interests"] = random.sample(User.interests, sample_size)
        return user

    def make_users(self):
        return self.make_docs(self._user_id_start, self._user_id_end)

    def make_docs(self, start_id, end_id):
        for i in range(start_id, end_id):
            yield self.make_one_user(i+1)
